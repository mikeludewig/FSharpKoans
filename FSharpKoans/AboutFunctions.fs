...
module ``03: Putting the Function into Functional Programming`` = 
    ...
    [<Test>]
    let ``03 The input to a function is a pattern (Part 2).`` () =   //_ wildcards accept any type could have entered a string
        (fun _ -> 75) 7 |> should equal 75

    [<Test>]
    let ``04 The input to a function is a pattern (Part 3).`` () =  //Prime number: number that has factors 1 and itself. (factor: 6 can be divided by 2 therefore 2 is a factor of 6)
    ...
    let ``10 A function can return a function (Part 1).`` () =        //
        ...
        let j = i 10   //10  passed into first parameter (love)
        let k = j 9    //9 passed into second first parameter
        k |> should equal 1
    ...
    *)
        ...
        let square v = v * v
        somefunc 3 square |> should equal 12 //12
        somefunc 3 ((*) 7) |> should equal 24 //24 //* operator is a function by itself
        somefunc 10 ((+) 8) |> should equal 28 //28 // + operator is a function in its own right
        somefunc 5 (fun z -> z + 22) |> should equal 32 //32
...
*)
    ...
    *)
    ...
    let ``24 The output type of one pipe must be the input type to the next`` () =
        ...
        3.0 |> a |> b |> should equal true    // input -> funciton -> funtion
    ...
    *)
    ...
    let ``33 Partially specifying arguments (Part 2).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let howl k = f k "slash/crunch/snap" //__ // <- multiple words on this line.  You MUST use `f`.
    ...
    let ``34 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows nguni = f "cow" (nguni + ", de gozaru")  // ",de gozaru")// <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
