    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            ...
            let rec ListLength count xs =
                match xs with
                |n::rest -> ListLength (count+1) rest
                |[] -> count
            ListLength 0 xs
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            ...
            let rec ListRev xs revxs = 
                match xs with
                |n::rest -> ListRev rest (n::revxs)
                |[] -> revxs
            ListRev xs []
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            ...
            let rec addone xs out=
                match xs with
                |n::rest -> addone rest ((n+1)::out)
                |[] -> List.rev (out)
            addone xs []
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            ...
            let rec double xs out=
                match xs with
                |n::rest -> double rest ((n*2)::out)
                |[] -> List.rev (out)
            double xs []
   ...
   *)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            ...
            let rec innerF xs out =
                match xs with
                |n::rest -> match f n with |true -> innerF rest (n::out)  |false -> innerF rest out 
                |[] -> List.rev (out)//out
            innerF xs []
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            ...
            let rec innerF xs out = 
                match xs with 
                |n::rest -> match n%2=0 with | false -> innerF rest (n::out) |true -> innerF rest out
                |[] -> List.rev(out) //ListRev out []
            innerF xs []
...
*)
    ...
    let ``14 A fold which sums a list`` () =
        let fold initialState xs =
            ...
            let rec innerF xs sum =
                match xs with
                |n::rest -> innerF rest (n+sum)
                |[] -> sum
    ...
    let ``15 A fold which multiplies a list`` () =
        let fold initialState xs =
            ...
            let rec innerF xs product =
                match xs with
                |n::rest -> innerF rest (n * product)
                |[] -> product
            innerF xs initialState
    ...
    let ``16 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            ...
            let rec innerF xs result =
                match xs with
                |n::rest -> innerF rest (f result n )
                |[] -> result
            innerF xs initialState
        fold (+) 0 [1;2;3;4] |> should equal 10
