...
module ``13: On the Record`` =
    ...
    let ``03 Decomposing with a record pattern`` () =
        ...
        let {Title=a; Author=_; Year=b} = book
    ...
    *)
    ...
    type MyRecord<'x, 'z> = {
        Who : 'x//string* char //FILL_ME_IN // <-- should be generic
        What : 'z//float * bool // <-- should be generic, and a different type to Who
        Where : string
    }
    ...
    let ``08 Creating a generic record`` () =
        ...
        let a = {MyRecord.Who = "The Doctor"; What = 4.53; Where = "TTFN"}
        let b = {MyRecord.Who = 'R'; What = false; Where = "tiffin"} 
